...
int main()
{
    ...
    vector<string> words = { "ability", "able", "about", "above", "accept", "according", "account",
    ...
    "gas", "general", "generation", "get", "girl", "give", "glass",
    "go", "goal", "good", "government", "great", "green", "ground", "group", "grow",
    ...
    "least", "leave", "left", "leg", "legal", "less", "let", "letter", "level", "lie", "life",
    ...
    "majority", "make", "man", "manage", "management", "manager", "many", "market", "marriage",
    ...
    "nice", "night", "no", "none", "nor", "north", "not", "note", "nothing", "notice", "now",
    "number", "occur", "of", "off", "offer", "office", "officer", "official", "often",
    ...
    "together", "tonight", "too", "top", "total", "tough", "toward", "town", "trade",
    ...
    "woman", "wonder", "word", "work", "worker", "world", "worry", "would", "write",
    "writer", "wrong", "yard", "yeah", "year", "yes", "yet", "you", "young", "your",
    "yourself" };
    int vieti = 3,nr=0,solutie=1;
    int cuvant = (rand() % 1000);
    std::vector<int> check(20, 0);
    check[0] = 1;
    vector<string> Incercate(26);
    char Litera;
    while (vieti > 0 && solutie==1)
    {
        solutie = 0;
        int gasit = 0;
        for (int i = 0; i < words[cuvant].size(); i++)
            if (check[i] == 1)
                cout << words[cuvant][i];
            else cout << "*";
        cout << endl;
        cout << "Mai ai " << vieti << " incercari" << endl;
        cout << "Introdu litera:";
        cin >> Litera;
        for (int i = 1; i < words[cuvant].size(); i++)
            if (Litera == words[cuvant][i] && check[i] == 0)
            {
                check[i] = 1;
                gasit = 1;
            }
    ...
    {
        cout << "Ai pierdut , cuvantul a fost: '"<< words[cuvant]<<"' incearca iar" << endl;
        main();
    }
    return 0;
}